...
{	
	...
	{
		mapDesc->BGMSoundNum = 1;
		//mapDesc->Fov = 80.f;
	}
	//========== Dynamic Login Title ==========//
	else if (MAINGAME->GetCurStateNum() == eGAMESTATE_TITLE)
	{
		mapDesc->BGMSoundNum = 1;
		switch (MapNum)
		{
			case 20:
			{
				strcpy(mapDesc->MapFileName, "20.map");
			} break;
			case 21:
			{
				mapDesc->bSky = 1;
				strcpy(mapDesc->SkyMod, "19_sky.mod");
				mapDesc->fogdesc.bEnable = TRUE;
				mapDesc->fogdesc.dwColor = 0xAFAFAFAA;
				mapDesc->fogdesc.fDensity = 0.5f;
				mapDesc->fogdesc.fStart = 4000;
				mapDesc->fogdesc.fEnd = 16000;
				mapDesc->backColor = 0xAFAFAFAA;
			} break;
			case 46:
			{
				mapDesc->DefaultSight = 18000;
			} break;
			case 54:
			{
				mapDesc->fogdesc.fDensity = 0.1f;
				mapDesc->fogdesc.fStart = 6000;
				mapDesc->fogdesc.fEnd = 16000;
			} break;
			case 56:
			{
				mapDesc->fogdesc.bEnable = FALSE;
			} break;
			case 72:
			{
				mapDesc->fogdesc.fStart = 12000;
			} break;
		}
	}
...
#ifndef _FORMOON_
	//aziz add logic to set morning and evening
	
	else// (MAINGAME->GetCurStateNum() != eGAMESTATE_TITLE && MAINGAME->GetCurStateNum() != eGAMESTATE_CHARSELECT && MAINGAME->GetCurStateNum() != eGAMESTATE_CHARMAKE)
	{
		switch(MapNum){
		case Alker_Harbor:{
							if (st.wHour >= 6 && st.wHour < 7)
							{
								mapDesc->bSky = 1;
								strcpy(mapDesc->SkyMod,"skybox_overcast01.mod");//19_sky.MOD
								mapDesc->fogdesc.bEnable = TRUE;
								mapDesc->fogdesc.dwColor = 0xAFAFAFAA;
								mapDesc->fogdesc.fDensity = 0.5f;
								mapDesc->fogdesc.fStart = 4000;
								mapDesc->fogdesc.fEnd = 16000;
								mapDesc->backColor = 0xAFAFAFAA;
								mapDesc->BGMSoundNum = 2;
							}
							else if (st.wHour >= 7 && st.wHour < 18 )
							{
								mapDesc->bSky = 1;
								strcpy(mapDesc->SkyMod,"sky_winter_20.mod");
								//mapDesc->fogdesc.bEnable = TRUE;
								//mapDesc->fogdesc.dwColor = 0xAFAFAFAA;
								//mapDesc->fogdesc.fDensity = 0.5f;
								//mapDesc->fogdesc.fStart = 4000;
								//mapDesc->fogdesc.fEnd = 16000;
								//mapDesc->backColor = 0xAFAFAFAA;
								mapDesc->BGMSoundNum = 8;
							}
							else if (st.wHour >= 18 || st.wHour < 6 )
							{
								mapDesc->bSky = 1;
								strcpy(mapDesc->SkyMod,"52_sky.mod");//sky_54_nanju.mod
								mapDesc->fogdesc.bEnable = TRUE;
								mapDesc->fogdesc.dwColor = 0x00000000;
								mapDesc->fogdesc.fDensity = 5.5f;
								mapDesc->fogdesc.fStart = 100;
								mapDesc->fogdesc.fEnd = 16000;
								mapDesc->backColor = 0x00000000;
								mapDesc->BGMSoundNum = 1485;
							}
						  }break;
			case The_Town_of_Nera_Castle:{
							...
							else if (st.wHour >= 7 && st.wHour < 18 )
							{
								...
								strcpy(mapDesc->SkyMod,"sky_winter_20.mod");
								//mapDesc->fogdesc.bEnable = FALSE;
								//mapDesc->backColor = 0xFFFFFFFF;
								mapDesc->BGMSoundNum = 1234;
							}
							else if (st.wHour >= 18 || st.wHour < 6 )
							{
								mapDesc->bSky = 1;
