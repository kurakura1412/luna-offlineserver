...
// ---- skr sorting inven
	{AA_InvenSort, "AA_InvenSort"},
	{AA_weatheronoff, "AA_weatheronoff"},
...
// --- skr : add set
	...
	case 9004:
		{
			STORAGEMGR->CustomEventEX( lId, p, we);
		}
		break;
	}
}
...
// ---- skr sorting inven
void AA_InvenSort(LONG lId, void * p, DWORD we)
{
	if( p == NULL ){ 	return;}
	cDialog* dlgA = WINDOWMGR->GetWindowForID( XCI_DIALOG );
	cDialog* dlgB = WINDOWMGR->GetWindowForID( BS_STALLDLG );
	cDialog* dlgC = WINDOWMGR->GetWindowForID( SSI_STALLDLG );
	cDialog* dlgD = WINDOWMGR->GetWindowForID( DE_DEALERDLG );
	if( dlgA && dlgA->IsActive()) { return; }
	if( dlgB && dlgB->IsActive()) { return; }
	if( dlgC && dlgC->IsActive()) { return; }
	if( dlgD && dlgD->IsActive()) { return; }
	
	if( GAMEIN->GetInventoryDialog() ) {
		GAMEIN->GetInventoryDialog()->OnActionEvent_custom( lId, p, we );
	}
}
...
{
	if( p == NULL ){ 	return;}
	WEATHERMGR->pushupbtn_onoffweather(lId, p, we);
}
