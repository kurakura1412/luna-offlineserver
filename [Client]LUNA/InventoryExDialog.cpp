...
//	
	...
	if( MAP->GetMapNum() == StreetTournament )
	{
		...
		{
			CHATMGR->AddMsg( CTC_SYSMSG, "Your Can't use this item at Street Tournament" );
			return;
		}
	}
#endif
			...
			case ITEM_KIND_REBORN :
				{
					...
					const int RBMAP = MAP->GetMapNum() == 52;
					if( ! RBMAP )
					{
						CHATMGR->AddMsg( CTC_OPERATOR, "Please go to Nera Village to Reborn!" );
						return;
					}

					pItem->SetLock(TRUE) ;
					Set_QuickSelectedItem(pItem) ;
					WINDOWMGR->MsgBox( MBI_RESETLEVEL_AREYOUSURE, MBT_YESNO, CHATMGR->GetChatMsg(2507) ) ;
					return ;
				}
				return ;
#endif
...
// --- skr : warehouse
			case ITEM_KIND_EXTEND_STORAGE_SET1 :
			{
				BYTE byNum = 0 ;
				byNum = HERO->GetStorageNum() ;
				if( MAX_STORAGELIST_NUM > byNum || byNum >= MAX_STORAGELIST_NUMSET1 )
				{
					WINDOWMGR->MsgBox( MBI_RESETSKILL_AREYOUSURE, MBT_OK, CHATMGR->GetChatMsg(1243) ) ;
					return ;
				}
			...
			case ITEM_KIND_EXTEND_STORAGE_SET2 :
			{
				BYTE byNum = 0 ;
				byNum = HERO->GetStorageNum() ;
				if( MAX_STORAGELIST_NUMSET1 > byNum || byNum >= MAX_STORAGELIST_NUMSET2 )
				{
					WINDOWMGR->MsgBox( MBI_RESETSKILL_AREYOUSURE, MBT_OK, CHATMGR->GetChatMsg(1243) ) ;
					return ;
				}
			...
			case ITEM_KIND_EXTEND_STORAGE_SET3 :
			{
				BYTE byNum = 0 ;
				byNum = HERO->GetStorageNum() ;
				if( MAX_STORAGELIST_NUMSET2 > byNum || byNum >= MAX_STORAGELIST_NUMSET3 )
				{
					WINDOWMGR->MsgBox( MBI_RESETSKILL_AREYOUSURE, MBT_OK, CHATMGR->GetChatMsg(1243) ) ;
					return ;
				}
			...
			case ITEM_KIND_EXTEND_STORAGE_SET4 :
			{
				BYTE byNum = 0 ;
				byNum = HERO->GetStorageNum() ;
				if( MAX_STORAGELIST_NUMSET3 > byNum || byNum >= MAX_STORAGELIST_NUMSET4 )
				{
					WINDOWMGR->MsgBox( MBI_RESETSKILL_AREYOUSURE, MBT_OK, CHATMGR->GetChatMsg(1243) ) ;
					return ;
				}
			...
			case ITEM_KIND_MARRY_REQUEST:
				{
					...
					if( !pObject )
					{
						CHATMGR->AddMsg(CTC_SYSMSG, CHATMGR->GetChatMsg(2487)) ;
						return;
					}
					else
					{
						CPlayer* pRequestMarryToPlayer = ((CPlayer*)pObject) ;
						if( pObject->GetObjectKind() != eObjectKind_Player )
						{
							CHATMGR->AddMsg(CTC_SYSMSG, CHATMGR->GetChatMsg(2487));
							return;
						}
						else if(pRequestMarryToPlayer->GetMarryPlayerID() != 0)
						{
							CHATMGR->AddMsg( CTC_SYSMSG, CHATMGR->GetChatMsg(2488) );
							return;
						}
						else if(HERO->GetMarryPlayerID() != 0)
						{
							CHATMGR->AddMsg( CTC_SYSMSG, CHATMGR->GetChatMsg(2490) );
							return;
						}
						else if(HERO->GetGender() == pRequestMarryToPlayer->GetGender())
						{				
							CHATMGR->AddMsg( CTC_SYSMSG, CHATMGR->GetChatMsg(2489) );
							return;
						}
						
						MARRYMGR->RequestMarrySyn( pObject ) ;
						
					}
				}
				break;

			case ITEM_KIND_MARRY_ABORT:
				{
					if(HERO->GetMarryPlayerID() == 0)
					{
						CHATMGR->AddMsg(CTC_SYSMSG, CHATMGR->GetChatMsg(2498));
						return;
					}
					MARRYMGR->AbortMarrySyn();									
				}
				break;
			
			case ITEM_KIND_MARRY_SKILLUPGRADE:
				{
					if(HERO->GetMarryPlayerID() == 0)
					{
						CHATMGR->AddMsg(CTC_SYSMSG, CHATMGR->GetChatMsg(2498));
					...
					DWORD marryskillidx = 10001101;
					for (int i = 0 ; i <= 500 ; i = i+100)
					{
						//oldSkill = HERO->GetSkillTree().GetData(marryskillidx+i);
						const SKILL_BASE* oldSkill = SKILLTREEMGR->GetSkillBase(marryskillidx+i);
						if (oldSkill)
						{							
							if ( oldSkill->mLearnedLevel + 1> 3)
							{
								CHATMGR->AddMsg(CTC_SYSMSG, CHATMGR->GetChatMsg(2500));
								return;	
							}
						}
							
					}
					MARRYMGR->MarrySkillUpgradeSyn();								
				}
				break;
				//aziz add
			case ITEM_KIND_ALIAS:
				{
					ITEM_INFO* pInfo = ITEMMGR->GetItemInfo( pItem->GetItemIdx() );
					if(pInfo->SupplyValue == 0)
					{
						CHATMGR->AddMsg(CTC_SYSMSG, CHATMGR->GetChatMsg(2432));
						return;
					}
...
void CInventoryExDialog::SetMoney(MONEYTYPE Money)
{
