...
void CItemManager::NetworkMsgParse(BYTE Protocol,void* pMsg)
{
	...
	case MP_ITEM_MONEY_ERROR:		CHATMGR->AddMsg( CTC_SYSMSG, "Gold Max or something Error" );		break;
	...
	case MP_ITEM_REBORN_ACK :						Item_Reset_Level_Ack( pMsg ) ;					break;
	case MP_ITEM_REBORN_NACK :						Item_Reset_Level_Nack( pMsg ) ;					break;
#endif
...
// ---- skr sorting
	case MP_ITEM_INVENSORT_ACK:
		{		
			CInventoryExDialog * pInven = GAMEIN->GetInventoryDialog();
			if( pInven ){ pInven->RefreshInvenItem(); }
		}
		break;
	case MP_ITEM_INVENSORT_NACK:
		{			
		}
		break;
...
//			pItem->SetToolTipImageRGB( TTCLR_ITEM_CANNOTEQUIP );
	...
	{
		char buf[128];
		MONEYTYPE SellPrice = pItem->GetItemInfo()->SellPrice;
...
{
	{
		...
		while( ! file.IsEOF() )
		{
			...
			{
				char szText[256];
				sprintf(szText, "Item already exists / Duplicated ID: %d", pInfo->ItemIdx);
			...
			{
				char szText[256];
				sprintf(szText, "Items sales amount is higher than the purchase amount ID: %d", pInfo->ItemIdx);
				char	szCaption[512];														// 아래 두줄 살짝 추가. -_-; 2007.05.28 이진영.
				sprintf( szCaption, "%s, %s() %d", __FILE__, __FUNCTION__, __LINE__); 
				MessageBox(NULL, szText, szCaption, NULL);
			}
			...
			pInfo->dwBuyVipPoint =  file.GetDword();
			//aziz Kill Shop 30 Sep
			pInfo->dwBuyKillPoint =  file.GetDword();
			//aziz Reborn Point 13 Oct
			pInfo->dwBuyRebornPoint =  file.GetDword();
#endif
...
//#ifndef _FORMOON_
		//[02-02-2012] Death and it for let Dialog when chat with NPC close after storage opened
...
#ifndef _FORMOON_
					case ITEM_KIND_HPMP_RECOVER_PERCENT:
#endif
						{
							EFFECTMGR->StartHeroEffectProcess(
								FindEffectNum("ItemUSE_PotionHP_little.beff"));
...
void CItemManager::Item_Sell_Ack( void* pMsg )
{
	...
	if( IsDupItem( pmsg->wSellItemIdx ) && ( pItem->GetDurability() - pmsg->SellItemNum ) > 0 )
	{
		...
		MONEYTYPE SellPrice = pSellItemInfo->SellPrice;
...
void CItemManager::Item_Buy_Nack( void* pMsg )
{
	...
	case NOT_VIPPOINT:
		CHATMGR->AddMsg( CTC_SYSMSG, CHATMGR->GetChatMsg( 1919 ) );
		CHATMGR->AddMsg( CTC_OPERATOR, "Not Enough VIP Point. Please Check your VIP Point before Purchase!" );
		break;
	//aziz Kill Shop 30 Sep
	case NOT_KILLPOINT:
		CHATMGR->AddMsg( CTC_SYSMSG, CHATMGR->GetChatMsg( 1919 ) );
		CHATMGR->AddMsg( CTC_OPERATOR, "Not Enough KILL Point. Please Check your KILL Point before Purchase!" );
		break;
	//aziz Reborn Point 13 Oct
	case NOT_REBORNPOINT:
		CHATMGR->AddMsg( CTC_SYSMSG, CHATMGR->GetChatMsg( 1919 ) );
		CHATMGR->AddMsg( CTC_OPERATOR, "Not Enough REBORN Point. Please Check your REBORN Point before Purchase!" );
		break;
#endif
	default:
...
//aziz Reborn in Game 29 Sep
//void CItemManager::Item_Reset_Level_Ack( void* pMsg )
//{
//	if( !pMsg ) return ;
//	if( !HERO ) return ;
//	CHATMGR->AddMsg(CTC_SYSMSG, CHATMGR->GetChatMsg( 1283 ) );
//}
//
void CItemManager::Item_Reset_Level_Nack( void* pMsg ) 
{
	WINDOWMGR->MsgBox( MBI_NOTICE, MBT_OK, CHATMGR->GetChatMsg( 1282 ) );	
}
void CItemManager::Item_Reset_Level_Ack( void* pMsg )
			...
			case ITEM_OPTION::Drop::KeyPercentManaRecovery:	
				{
					...
					if(info->EquipSlot == eWearedItem_Costume_Head ||
						info->EquipSlot == eWearedItem_Costume_Dress ||
						info->EquipSlot == eWearedItem_Costume_Glove ||
						info->EquipSlot == eWearedItem_Costume_Shoes )
					{
						textUnit.mColor = NAMECOLOR_WANTED;
					} else {
...
void CItemManager::AddPetSkillToolTip( cIcon& icon, const ITEMBASE& item )
{
	...
	if(pItemInfo->SupplyType != ITEM_KIND_PET) return;

	const DWORD petkindnya	= pItemInfo->SupplyValue;
	const PET_INFO* petinfox = PETMGR->GetPetInfo(petkindnya);

	if(!petinfox) return;

	const DWORD petskillnya	= petinfox->StaticItemIndex;
	const ITEM_INFO* infoskill	= GetItemInfo( petskillnya );
	...
	char bufpetx[128] = {0,};
	DWORD colorx = RGB_HALF(0, 255, 0);

	icon.AddToolTipLine( "Additional Skill:", colorx );

	sprintf( bufpetx, "- %s", infoskill->ItemName );
	icon.AddToolTipLine( bufpetx, colorx );
#ifdef _GMTOOL_
	icon.AddToolTipLine( "" );
	char bufpetx2[128] = {0,};
	icon.AddToolTipLine( "Pet Group:", colorx );

	sprintf( bufpetx2, "%s", petinfox->Name );
	icon.AddToolTipLine( bufpetx2, colorx );
#endif
}
...
#ifndef _FORMOON_
//aziz Reborn in Game 29 Sep
void CItemManager::UseItem_ResetLevel(CItem* pItem)
{
	if( !pItem ) return ;																// 아이템의 무결성을 체크한다.
...
void CItemManager::UseItem_ItemMallItem(CItem* pItem, CObject* pTarget)
{
	...
	//WINDOWMGR->MsgBox( MBI_NOTICE, MBT_OK, "Sent UseItem_ItemMallItem to Server" );
...
#else
	char buffnamanya[MAX_NAME_LENGTH+1] = "";
	SafeStrCpy( buffnamanya, pName, MAX_NAME_LENGTH+1 );
	const ITEM_INFO* pInfo = GetItemInfo( dwItemIdx );
	if( NULL == pInfo )
	{
		return;
	}
    char buf[256] = {0};
	sprintf( buf, CHATMGR->GetChatMsg(2285), buffnamanya, pInfo->ItemName, dwEnchantLevel );
	CHATMGR->AddMsg( CTC_ENCHANT, buf );
#endif
}
